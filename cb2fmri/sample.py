...
from cb2fmri.utils import N_LANDMARKS, ID_TO_ASSET, LANDMARK_NAME_TO_TEXT, N_RUNSETS, N_TRIALS, open_browser, \
    get_distance, set_difficulty
...
def clean_scenario(scenario):
    ...
    scenario['duration_s'] = TIME_LIMIT
...
def sample_landmarks(n_landmarks, target_card, assets, available_cards, temp=1):
    ...
    for i in range(n_landmarks):
        _assets = [
            dict(card=x, d=get_distance(x['cell']['coord'], target_loc)) for x in assets if x not in assets_used
        ]
        ...
        landmark, distance = s['card'], s['d']
        landmarks.append((landmark, distance))
        assets_used.append(landmark)
        if i == 0:
            for card_id in available_cards:
                card = available_cards[card_id]
                d = get_distance(card['prop_info']['location'], landmark['cell']['coord'])
                if d == distance:
                    excluded_distractor_card_ids.add(card['id'])
